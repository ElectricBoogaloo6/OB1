...
"""
the parameters for analysis are set using the "return_params()" function. 
The scripts utilises the "get_results()" function which takes in 3 inputs, the input text file name, all data file, and unrecognized words file. 
It reads the text file and splits it by spaces to get a list of words. 
Then, it appends all non-empty words to a list called "individual_words" and creates a dataframe from it. It also has specific functionality, 
for example: if the language is set to German, it uses the "get_freq_and_pred()" function to get frequency and prediction values for the words.
"""

def get_results(input_text_filename,input_file_all_data,input_file_unrecognized_words):
    with open(input_file_all_data,"rb") as f: #NV: changed "r" to "rb"
        with open(input_file_unrecognized_words,"rb") as g: #NV: changed "r" to "rb"
...
#                    word_pred_values[:] = 0.1
                ...
                # TODO fix
                #import copy_reg
                df_freq_pred = df_freq_pred.iloc[0:len(df_individual_words),:] #NV: ik snap niet wat hier moet gebeuren
                df_individual_words = pd.concat([df_individual_words,df_freq_pred],axis=1) #removed join_axes=[df_individual_words.index]
...
def get_results_simulation(task,input_file_all_data,input_file_unrecognized_words):
    # MM: task can be "Sentence" or "Flanker" (see main_exp)
    #NV: added EmbeddedWords
